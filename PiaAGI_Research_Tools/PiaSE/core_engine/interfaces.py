...
import uuid # Added for action_id
...
class DetectedObject(BaseDataModel):
    ...
    bounding_box: Optional[Tuple[float, float, float, float]] = None # e.g., (x_min, y_min, x_max, y_max) normalized
    attributes: Dict[str, Any] = Field(default_factory=dict) # e.g., {"color": "red", "state": "open"}
    distance: Optional[float] = None
    orientation: Optional[Tuple[float,float,float,float]] = None # Quaternion or Euler

class AgentStatePercept(BaseDataModel):
    position: Optional[Tuple[float, ...]] = None
    orientation: Optional[Tuple[float, ...]] = None
    inventory: Optional[List[str]] = None # Could be List[InventoryItem] in a future iteration
    internal_stats: Dict[str, Any] = Field(default_factory=dict) # e.g., health, energy, custom status

# For ActionCommand parameters
class MoveParams(BaseDataModel):
    direction: Optional[str] = None
    target_coordinates: Optional[Tuple[float, ...]] = None
    speed_profile: Optional[str] = "normal" # e.g., "slow", "normal", "fast"

class InteractParams(BaseDataModel):
    object_id: str
    interaction_type: Optional[str] = None # e.g., "open", "take", "use_on_target"
    tool_id: Optional[str] = None # if using a tool for interaction

# Modified Percept Types
class VisualPercept(BaseDataModel):
    image_url: Optional[str] = None
    raw_image_data: Optional[bytes] = None
    detected_objects: Optional[List[DetectedObject]] = None # Modified

class AuditoryPercept(BaseDataModel):
    sound_url: Optional[str] = None
    raw_sound_data: Optional[bytes] = None
    transcribed_text: Optional[str] = None
    speaker_id: Optional[str] = None
    emotion_hint: Optional[str] = None # Added
    prosody_features: Optional[Dict[str, float]] = None # Added
...
class ActionCommand(BaseDataModel):
    action_id: str = Field(default_factory=lambda: str(uuid.uuid4())) # Added
    action_type: str
    generic_parameters: Dict[str, Any] = Field(default_factory=dict) # Renamed
    move_params: Optional[MoveParams] = None # Added
    interact_params: Optional[InteractParams] = None # Added
    timeout_seconds: Optional[float] = None # Added

class ActionResult(BaseDataModel):
    timestamp: float
    status: str  # e.g., "success", "failure", "pending"
    new_perception_snippet: Optional[PerceptionData] = None
    message: Optional[str] = None
    reward: float = 0.0 # Added
    is_terminal: bool = False # Added
    achieved_goal_ids: List[str] = Field(default_factory=list) # Added
    failure_reason_code: Optional[str] = None # Added
    custom_details: Dict[str, Any] = Field(default_factory=dict) # Renamed
...
class Environment(ABC):
    ...
    def get_spontaneous_events(self) -> List[PiaSEEvent]:
        """Returns a list of spontaneous events generated by the environment since the last call."""
        return [] # Default empty implementation

    def add_entity(self, entity_type: str, entity_id: str, config: Dict[str, Any]) -> bool:
        """Adds a new entity to the environment. Returns True on success."""
        return False

    def remove_entity(self, entity_id: str) -> bool:
        """Removes an entity from the environment. Returns True on success."""
        return False

    def update_entity_state(self, entity_id: str, new_state: Dict[str, Any]) -> bool:
        """Updates the state of an existing entity. Returns True on success."""
        return False
...
class AgentInterface(ABC):
    ...
    def initialize_q_table(self, state: any, action_space: list):
        ...
        # Note: Specific to QLearningAgent implementations.
        pass
    ...
    def configure(self, config: Dict[str, Any], **kwargs) -> None:
        """
        Optional method to configure the agent with parameters, typically at initialization
        or when a DSE applies agent-specific overrides for a curriculum step.
        'env_info' and 'action_space' might be passed via kwargs by the engine.
        """
