...
评估不同的提示词策略模板对于实验对象[GPT-3.5] 表现的有效性，通过自主设计实验方法，生成3条不同的测试数据，进行实验、记录、评分及分析，并以表格和图表的形式表达。
...
将测试数据输入[实验对象]，记录生成的回复。

### 评分和记录

由[实验对象]对每个生成的回复进行评分，记录评分结果。

### 分析

汇总评分结果，使用图表和表格比较不同提示词策略模板的性能。
